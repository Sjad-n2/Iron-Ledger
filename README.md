...
- Opens on Log Session first and prompts new users to log in/sign up or continue as guest.
...
- Reuses the last saved session date when you save again with the date field blank.
...
Recommended: Cloudflare Pages. If you want to keep using a `workers.dev` URL, use the Workers setup below instead.
...
If your URL ends in `workers.dev`, you are probably using Cloudflare Workers instead of Cloudflare Pages. In that case:

1. Copy `wrangler.worker.example.toml` to `wrangler.toml`.
2. Replace `replace-with-your-d1-database-id` with your D1 database ID.
3. Keep `worker.js`, `functions/api/[[path]].js`, `index.html`, `app.js`, `styles.css`, and the `assets` folder in the upload.
4. Deploy the Worker.

Cloudflare Workers Static Assets serve files from the configured assets directory, and the `worker.js` file routes `/api/*` to the login backend. The Worker config uses `run_worker_first = true` so protected source/config files are blocked before static assets are served.
