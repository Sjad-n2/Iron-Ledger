...
const ENTRY_CHOICE_KEY = "iron-ledger-entry-choice";
const LAST_SESSION_DATE_KEY = "iron-ledger-last-session-date";
...
function resolveSessionDate() {
  const chosen = $("#sessionDate")?.value || "";
  const last = getStoredValue(LAST_SESSION_DATE_KEY);
  return chosen || last || todayIso();
}
...
async function apiJson(path, options = {}) {
  ...
  if (!contentType.includes("application/json")) {
    const error = new Error("Login is not deployed on this site yet. Upload the functions folder and deploy with Cloudflare Pages Functions or Workers Assets.");
    error.status = 0;
    throw error;
  }
...
function renderAccount() {
  ...
  status.textContent = signedIn
    ...
    : state.cloudAvailable
      ...
      : state.accountSetupMessage || "Login is not set up on this deployment yet. You can still save workouts on this device.";
}

function showEntryPromptIfNeeded() {
  const prompt = $("#entryPrompt");
  if (!prompt) return;
  const choice = getStoredValue(ENTRY_CHOICE_KEY);
  prompt.hidden = Boolean(choice || state.account);
}

function closeEntryPrompt(choice) {
  setStoredValue(ENTRY_CHOICE_KEY, choice);
  const prompt = $("#entryPrompt");
...
async function initAccount() {
  try {
    ...
    state.accountSetupMessage = "";
    state.account = data.user || null;
    if (state.account) setStoredValue(ENTRY_CHOICE_KEY, "account");
    renderAccount();
    showEntryPromptIfNeeded();
    ...
    state.accountSetupMessage =
      error.status === 503
        ? "Login backend found, but the D1 database binding IRON_LEDGER_DB is missing. Add the binding in Cloudflare, then redeploy."
        : error.message || "Login is not set up on this deployment yet. You can still save workouts on this device.";
    renderAccount();
    showEntryPromptIfNeeded();
  }
}
...
async function handleAccountAuth(mode) {
  ...
  try {
    ...
    setStoredValue(ENTRY_CHOICE_KEY, "account");
    $("#accountPassword").value = "";
    renderAccount();
    await pullCloudSessions();
    await syncLocalToAccount();
  } catch (error) {
    state.cloudAvailable = error.status !== 0;
    state.accountSetupMessage =
      error.status === 503
        ? "Login backend found, but the D1 database binding IRON_LEDGER_DB is missing. Add the binding in Cloudflare, then redeploy."
        : error.message;
...
function rowsToMarkdown(rows, options = {}) {
  ...
  const date = options.date || (hasDocument ? resolveSessionDate() : "") || todayIso();
...
async function saveParsedSession() {
  if (!state.parsed.length && $("#rawNotes").value.trim()) {
    state.parsed = parseRawNotes($("#rawNotes").value);
    renderParsed(state.parsed);
  }
  if (!state.parsed.length) {
    $("#parseStatus").textContent = "Add notes first, then parse or save.";
    return;
  }
  const date = resolveSessionDate();
  ...
  saveLocalSessions(sessions);
  setStoredValue(LAST_SESSION_DATE_KEY, date);
...
function setView(viewName) {
  ...
  const subtitles = {
    dashboard: "Latest tracked session",
    stats: "Filter progress by date, muscle, exercise, and metric.",
    history: "Review and delete saved sessions.",
    log: "Write rough notes, parse them, then save.",
    exercises: "All tracked movements in one place.",
    notes: "Focus points and calculation caveats.",
    account: "Log in to sync, or keep using guest mode."
  };
  $("#pageTitle").textContent = titles[viewName] || "Iron Ledger";
  $("#sessionSubtitle").textContent = subtitles[viewName] || "";
}
...
function bindEvents() {
  ...
  $("#entryLoginButton").addEventListener("click", () => {
    closeEntryPrompt("login");
    setView("account");
    $("#accountEmail").focus();
  });
  $("#entryGuestButton").addEventListener("click", () => closeEntryPrompt("guest"));
  ...
  $$("[data-save-session-shortcut]").forEach((button) => {
    button.addEventListener("click", saveParsedSession);
  });
...
$("#sessionDate").value = getStoredValue(LAST_SESSION_DATE_KEY);
